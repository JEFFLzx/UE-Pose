# ------------------------------------------------------------------------------
# EDL Training Script
# Based on train-posehrnetfusion-nofusion-dropout.py
# Replaces MC Dropout with Evidential Deep Learning (EDL) uncertainty estimation
# ------------------------------------------------------------------------------
...
from core.function_edl import train_posehrnetfusion_nofusion_edl
from core.function_edl import validate_posehrnetfusion_nofusion_edl
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train keypoints network with EDL uncertainty')
...
def main():
    ...
    # EDL NIG Loss
    edl_loss_event = NIGLoss(reg_coeff=args.edl_reg_coeff).cuda()
    edl_loss_rgb = NIGLoss(reg_coeff=args.edl_reg_coeff).cuda()

    logger.info(f'EDL lambda: {args.edl_lambda}, EDL reg_coeff: {args.edl_reg_coeff}')
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        # train for one epoch with EDL
        train_posehrnetfusion_nofusion_edl(
        ...
        perf_indicator_event, perf_indicator_rgb = validate_posehrnetfusion_nofusion_edl(
