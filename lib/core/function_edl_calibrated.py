# ------------------------------------------------------------------------------
# Calibrated EDL Training and Validation Functions
# Based on function_edl.py with:
#   1. Calibration loss passed to NIGLossCalibrated during training
#   2. Temperature scaling applied during validation/inference
#   3. EPnP used for single-channel pose estimation instead of BPnP
# ------------------------------------------------------------------------------
...
def _epnp_solve_pose(keypoints_2d, landmarks_3d, intrinsics, dist_coeffs=None,
                     ransac_iters=10000, reproj_thresh=20.0):
    """
    Solve pose using EPnP with RANSAC for a single image.
    
    Args:
        keypoints_2d: (J, 2) array of 2D keypoint coordinates
        landmarks_3d: (J, 3) array of 3D landmark positions
        intrinsics:   (3, 3) camera intrinsic matrix
        dist_coeffs:  distortion coefficients (default: zeros)
        ransac_iters: number of RANSAC iterations
        reproj_thresh: reprojection error threshold for RANSAC
    
    Returns:
        pose_4x4: (4, 4) pose matrix, or identity if failed
    """
    if dist_coeffs is None:
        dist_coeffs = np.zeros(5)
    
    pts_3d = landmarks_3d.astype(np.float64)
    pts_2d = keypoints_2d.astype(np.float64).reshape(-1, 1, 2)
...
def train_posehrnetfusion_nofusion_edl_calibrated(config, 
                        ...
                        edl_lambda=0.1):
    """
    Training function for Calibrated EDL-based uncertainty estimation.
    
    Key difference from original: uses NIGLossCalibrated which includes:
      - Standard NIG NLL + evidence regularizer
      - Calibration loss: aligns predicted uncertainty with actual heatmap peak error
    """
    ...
    print("TRAINING NO FUSION - CALIBRATED EDL UNCERTAINTY")
    ...
    for i, (input_tensor_event, target_event, target_weight_event, trans_hm_to_img_event, trans_crop_to_img_event, input_tensor_rgb, target_rgb, target_weight_rgb, trans_hm_to_img_rgb, trans_crop_to_img_rgb, meta) in enumerate(train_loader_event_rgb):
        ...
        # Calibrated EDL loss: includes calibration term
        ...
        if nig_params_event is not None and edl_loss_event is not None:
            ...
            # Extract predicted heatmap peaks for calibration loss
            pred_coords_event = _extract_keypoint_coords_from_heatmap(output_event.detach())
            loss_edl_ev = edl_loss_event(
                nig_params_event, target_coords_event,
                pred_coords=pred_coords_event  # NEW: enables calibration loss
            )
        ...
        if nig_params_rgb is not None and edl_loss_rgb is not None:
            ...
            pred_coords_rgb = _extract_keypoint_coords_from_heatmap(output_rgb.detach())
            loss_edl_r = edl_loss_rgb(
                nig_params_rgb, target_coords_rgb,
                pred_coords=pred_coords_rgb  # NEW: enables calibration loss
            )
...
def validate_posehrnetfusion_nofusion_edl_calibrated(
    config, 
    val_loader, 
    val_dataset, 
    model_event, model_rgb, model_hrnet_encoder_event, model_hrnet_encoder_rgb, 
    ...
    temperature=1.0):
    """
    Validation function with Calibrated EDL uncertainty.
    
    Key differences from original:
      - Uses temperature scaling for uncertainty calibration
      - Computes EPnP poses alongside BPnP for downstream comparison
    """
    batch_time = AverageMeter()
    losses_event = AverageMeter()
    acc_event = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i, (input_tensor_event, target_event, target_weight_event, trans_hm_to_img_event, trans_crop_to_img_event, input_tensor_rgb, target_rgb, target_weight_rgb, trans_hm_to_img_rgb, trans_crop_to_img_rgb, meta) in enumerate(val_loader):
            ...
            # EDL uncertainty with temperature scaling
            if nig_params_event is not None:
                epistemic_event = NIGLossCalibrated.compute_uncertainty(
                    nig_params_event, temperature=temperature)  # Temperature scaling
            ...
            # get the event poses (still BPnP for compatibility with pose_estimator)
            ...
            if nig_params_rgb is not None:
                epistemic_rgb = NIGLossCalibrated.compute_uncertainty(
                    nig_params_rgb, temperature=temperature)  # Temperature scaling
